...
#         if not plan:
#             # ── Conversational fallback (unchanged) ───────────────────────────
...
Architectural contracts enforced:
  ...
  - Security accessed via ctx.security — never imports security.auth directly.
  - Brain accessed via ctx.brain_router.plan() / .chat() — never imports brain/.
  - Multi-step plan execution delegated to AgentSupervisor (Phase 3).
    The Supervisor owns the execution loop, failure detection, reflection,
    and bounded retries. The Dispatcher only owns routing decisions.
...
class Dispatcher:
    ...
    Execution path:
        handle()
          ├─ shutdown check
          ├─ pending auth interception  → _resume_after_auth()
          ├─ pronoun resolution         → _resolve_pronouns()
          ├─ fast-path regex match      → _execute_intent()   (single tool)
          └─ LLM planning path         → _llm_pipeline()
               ├─ plan empty           → brain_router.chat()  (conversational)
               └─ plan non-empty       → AgentSupervisor      (supervised execution)
    """
    ...
    # ── Single-Intent Execution (fast-path only) ──────────────────────────────
    ...
    async def _execute_intent(
        ...
        """
        Executes a single resolved intent through the registry.
        Used exclusively by the fast-path — single tool, no reflection loop.

        Handles auth interception, last-opened-app tracking, and memory writes.
        Multi-step plans and failure recovery are handled by AgentSupervisor,
        not here.
        """
        result = await self._execute_step(tool_name, args, req_id, ctx)
    ...
    async def _llm_pipeline(
        ...
        1. Calls brain_router.plan() to generate a tool execution plan.
        2. If plan is empty → conversational fallback via brain_router.chat().
        3. If plan is non-empty → delegates to AgentSupervisor for supervised
           execution with reflection-and-retry on failure.

        The Dispatcher owns routing decisions.
        AgentSupervisor owns plan execution, failure detection, and recovery.
        """
        history          = ctx.memory.conversation.get_history()
        tool_definitions = ctx.tool_registry.get_all_definitions()
        ...
        # AgentSupervisor takes ownership of the execution loop.
        # It executes steps through ctx.tool_registry (preserving all security
        # gates), detects failures, requests LLM reflection, and retries up to
        # MAX_RETRIES times before aborting.
        #
        # The Dispatcher receives a single structured result back and writes
        # it to conversation memory. No execution logic lives here.
        logger.info(
            f"[{req_id}] SUPERVISOR_DELEGATE | "
            f"Handing {len(plan)} step(s) to AgentSupervisor."
        )

        supervisor = AgentSupervisor(ctx=ctx, req_id=req_id)

        result = await supervisor.execute_with_reflection(
            user_input   = user_text,
            initial_plan = plan,
        )

        # ── Memory write ──────────────────────────────────────────────────────
        # Supervisor handles its own trace writes to session state.
        # Dispatcher writes the final user/assistant exchange to conversation
        # memory so the turn appears in future LLM context.
    ...
    async def _execute_step(
        ...
        """
        Executes a single tool through the registry with timeout protection.
        Used by _execute_intent() (fast-path single tools only).

        Never raises — always returns a structured result dict.
        Timeout sourced from settings, not hardcoded.

        Note: AgentSupervisor has its own equivalent _execute_step() method
        that it uses for plan execution. Both route through
        ctx.tool_registry.execute_tool() so all gates are enforced identically.
        """
        logger.info(f"[{req_id}] EXEC | tool={tool_name} args={args}")
